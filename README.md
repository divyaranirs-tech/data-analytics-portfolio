# data-analytics-portfolio

Hi — I'm Divya, an aspiring data analyst. Below are my selected projects. Click any project to see code, README, screenshots and sample data.

## Projects

1. **Healthcare Analytics (Excel)**  
   🔗   
   _Short: Dashboard analyzing patient admissions & KPIs._

2. **World Energy Consumption (Power BI)**  
   🔗  
   _Short: Power BI report exploring energy consumption by country & year._

3. **Bank Marketing Analysis (SQL)**  
   🔗 
   _Short: SQL analysis identifying customer segments & campaign success._

   

---

## Contact
- Email: divyaranirs@gmail.com 
- LinkedIn: # 📊 Divya's Data Analytics Portfolio

Hi — I'm Divya, an aspiring data analyst. Below are my selected projects. Click any project to see code, README, screenshots and sample data.

## Projects

1. **Healthcare Analytics (Excel)**  
   🔗 https://github.com/yourname/Healthcare-Analytics-Excel  
   _Short: Dashboard analyzing patient admissions & KPIs._

2. **World Energy Consumption (Power BI)**  
   🔗 https://github.com/yourname/World-Energy-PowerBI  
   _Short: Power BI report exploring energy consumption by country & year._

3. **Bank Marketing Analysis (SQL)**  
   🔗 https://github.com/yourname/Bank-Marketing-SQL  
   _Short: SQL analysis identifying customer segments & campaign success._

4. **Adult Census Dashboard (Tableau)**  
   🔗 https://github.com/yourname/Adult-Census-Tableau  
   _Short: Demographic analysis and interactive Tableau dashboard._

---

## Contact
- Email: your.email@example.com  
- LinkedIn: https://www.linkedin.com/in/divya-rani-2b9a7a219



